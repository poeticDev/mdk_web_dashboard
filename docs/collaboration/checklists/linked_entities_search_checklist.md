...
- [x] Provider 기반 재사용 예제/샘플 Widget 작성

## 공통 검색 UI 컴포넌트
- [x] `EntitySearchField` 위젯 (텍스트 필드 + 팝오버) 구현
- [x] ModalBottomSheet(모바일) 대응 UI 구현
- [x] 접근성(키보드 탐색/ARIA/Semantics) 적용
- [x] 위젯 테스트/골든 스냅샷 추가

## Classroom Timetable Modal 적용
- [x] 학과 입력 필드 → 검색 컴포넌트로 교체 (`departmentId` 연동)
- [x] 담당자 입력 필드 → 검색 컴포넌트로 교체 (`instructorId` 연동)
- [x] 선택/초기화/검증 로직 업데이트 (SnackBar/Validation)
- [ ] 회귀 테스트(수동/자동)로 기존 기능 영향 확인

## 검증 & 문서
- [x] 새 Repository/Controller/Widget 단위 테스트 통과 (`flutter test`)
